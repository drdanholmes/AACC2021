...
Creating interactive tables is easy with the **DT** package.

```{r summary}
sum_table <- qc_data %>%
  group_by(hospital, instr_code, test_code, qc_name) %>%
...
Creating interactive figures is easy with the **plotly** package.
...
cr_stats<- qc_data %>%
  filter(hospital == "SPH", test_code == "CRQ") %>%
  group_by(qc_name) %>%
  summarise(obs_mean = round(mean(result),1), obs_sd = round(sd(result),1), obs_cv = round(obs_sd/obs_mean*100, 1), set_mean = round(mean[1],1), set_sd = sd[1], set_cv = round(set_sd/set_mean*100,1))
